                           ...
                           int signaled, uint64_t wr_id)
{
    ...
    if (rc) {
        MRC_LOG_ERROR("post WRITE rc=%d errno=%d (bad=%p)", rc, errno,
                      (void *) bad);
    }
    return rc;
}
                         ...
                         int warmup)
{
    ...
    for (int i = 0; i < warmup; i++) {
        ...
        if (n < 0 || wc.status != IBV_WC_SUCCESS) {
            MRC_LOG_ERROR("Warmup send wc failed: %s",
...
{
    (void) res;
    MRC_LOG_DEBUG("Warmup receiver - allowing %d warmup ops to complete",
                  warmup);
    return 0;
}
                                ...
                                uint32_t peer_rkey, struct bench_stats *out)
{
    ...
    if (cq_mod <= 0 || cq_mod >= tx_depth) {
        MRC_LOG_ERROR("CQ_MOD(%d) must be in (0, TX_DEPTH(%d))", cq_mod,
                      tx_depth);
        return -1;
    }
    ...
    while (totscnt < tot_iters || totccnt < tot_iters) {
        while (totscnt < tot_iters && totscnt < (uint64_t) tx_depth + totccnt) {
            int should_signal = (((uint64_t) totscnt % (uint64_t) cq_mod) ==
                                 (uint64_t) (cq_mod - 1)) ||
                                (totscnt == tot_iters - 1);
    ...
    // Send one-byte RDMA write with immediate as signal after benchmark poll
    // done
...
static void validate_received_data(char *recv_buf, size_t msg_size)
{
    ...
    if (errors == 0)
        ...
        MRC_LOG_ERROR("Data validation found %d errors (%zu bytes checked)",
                      errors, check);
}
...
static void usage(const char *argv0)
{
    ...
    printf(
        "  -w, --warmup-iters=<n> number of warmup iterations (default %d)\n",
        DEF_WARMUP_ITERS);
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "p:d:i:g:m:n:s:S:F:w:h", long_options,
                        NULL);
        if (c == -1)
            break;
    ...
    if (min_msg_size > max_msg_size) {
        fprintf(stderr,
                "Error: min-size (%zu) cannot be larger than size (%zu)\n",
    ...
    if (max_msg_size > DEF_SWEEP_MAX_MSG_SIZE) {
        MRC_LOG_WARN(
            "msg_size %zu exceeds DEF_SWEEP_MAX_MSG_SIZE %zu, clamping",
    ...
    int supported = 0;
    if (mrc_query_device(res.ibv_context, &mrc_attr, &supported) ||
        !supported) {
    ...
    // --- Context, PD, CQ, QP ---
    struct mrc_context_attr ctx_attr = {.mrc_api_version_used = MRC_PROTOCOL_VERSION_1};
    ...
    res.mrc_qp =
        create_mrc_qp(res.mctx, res.mrc_cq, res.mrc_recv_cq, res.ibv_pd);
    if (!res.mrc_qp) {
        cleanup_resources(&res);
        return 1;
    }
    ...
    while (true) {
        ...
        if (!is_client) {
            MRC_LOG_DEBUG("Receiver: posting RECV for write-with-immediate "
                          "signal (before sync)");
            char *dummy_recv_buf = (char *) res.buffer + 2 * cur_msg_size;
            if (post_recv(res.mrc_qp, dummy_recv_buf, 0, res.mr->lkey,
                          0x1000)) {
        ...
        if (is_client) {
            ...
            if ((rc = warmup_sender(&res, cur_msg_size, peer_recv_addr,
                                    peer.rkey, warmup_iters)))
                break;
            if ((rc = run_benchmark_sender(&res, cur_msg_size, iters,
                                           peer_recv_addr, peer.rkey, &st)))
                break;
        } else {
