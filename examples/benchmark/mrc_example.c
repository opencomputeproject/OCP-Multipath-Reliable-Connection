...
static inline int change_mrc_qp_to_init(struct mrc_qp *mrc_qp) {
  ...
  struct mrc_qp_attr mrc_attr = (struct mrc_qp_attr){0};
  ...
  int ret = mrc_modify_qp(mrc_qp, &ibv_attr, ibv_mask, &mrc_attr, mrc_mask);
  if (ret) {
...
// ---------- SEND helpers ----------
static inline int post_rdma_write(struct mrc_qp *qp, uint64_t buf, uint32_t len,
                                  uint32_t lkey, uint64_t remote_addr,
                                  uint32_t rkey, int signaled, uint64_t wr_id) {
...
static inline int post_rdma_write_with_imm(struct mrc_qp *qp, uint64_t buf,
...
static inline int post_recv(struct mrc_qp *qp, uint64_t buf, uint32_t len,
                            uint32_t lkey, uint64_t wr_id) {
  struct ibv_sge sge = {.addr = buf, .length = len, .lkey = lkey};
...
static inline int warmup_sender(struct mrc_resources *res, size_t msg_size,
  ...
  for (int i = 0; i < warmup; i++) {
    ...
    if (post_rdma_write(res->mrc_qp, (uint64_t)send_buf, (uint32_t)msg_size,
...
static inline int warmup_receiver(struct mrc_resources *res, int warmup) {
  (void)res;
...
// ---------- benchmark loops ----------
static inline int
run_benchmark_sender(struct mrc_resources *res, size_t msg_size, int iters,
                     uint64_t peer_recv_addr, uint64_t peer_signal_addr,
                     uint64_t local_signal_addr, uint32_t peer_rkey,
  ...
  // Send RDMA write with immediate as signal after benchmark.
  MRC_LOG_DEBUG("Sending 8-byte write-with-immediate signal");
  if (post_rdma_write_with_imm(res->mrc_qp, local_signal_addr, sizeof(uint64_t),
                               res->mr->lkey, peer_signal_addr, peer_rkey,
                               (uint32_t)msg_size, 0xFFFFFFFFULL)) {
    free(wc);
    return -1;
  }
...
static inline int run_benchmark_receiver(struct mrc_resources *res, int iters,
                                         size_t msg_size,
                                         struct bench_stats *out) {
  (void)iters;
  (void)msg_size;
  ...
  // RECV already posted before sync point to avoid RNR
...
static void validate_received_data(char *recv_buf, size_t msg_size) {
  ...
  if (errors == 0)
    MRC_LOG_DEBUG("Data validation passed: no errors in %zu bytes", check);
  else
    MRC_LOG_ERROR("Data validation found %d errors (%zu bytes checked)", errors,
                  check);
}
...
int main(int argc, char **argv) {
  ...
  // --- MRC caps ---
  struct mrc_device_attr mrc_attr = {0};
  int supported = 0;
  if (mrc_query_device(res.ibv_context, &mrc_attr, &supported) || !supported) {
  ...
  // --- Buffer & MR ---
  const size_t buf_size = 2 * max_msg_size + 2 * sizeof(uint64_t);
  ...
  while (true) {
    ...
    char *send_buf = (char *)res.buffer;
    char *recv_buf = (char *)res.buffer + max_msg_size;

    // Only the sender fills the send buffer with the test pattern
    if (is_client) {
      for (size_t i = 0; i < cur_msg_size; i++)
        send_buf[i] = (char)(i & 0xFF);
    }

    // Only the receiver clears the receive buffer
    if (!is_client) {
      memset(recv_buf, 0, cur_msg_size);
    }

    // Receiver posts RECV work request BEFORE synchronization to avoid RNR
    if (!is_client) {
      MRC_LOG_DEBUG("Receiver: posting RECV for write-with-immediate "
                    "signal (before sync)");
      if (post_recv(res.mrc_qp, 0, 0, res.mr->lkey, 0x1000)) {
        MRC_LOG_ERROR("Receiver: failed to post RECV");
        rc = -1;
        break;
      }
    }
    // Receiver will do an rdma write to the firsthalf of the signal buffer
    // Sender will keep polling until the firsthalf of signal = msg_size_ctr.
    MRC_LOG_DEBUG("SYNC POINT: (is_client=%d, msg_size=%zu)...", is_client,
                  cur_msg_size);
    if (!is_client) {
      volatile uint64_t *signal_buf =
          (uint64_t *)((char *)res.buffer + 2 * max_msg_size);
      *signal_buf = msg_size_ctr;
      post_rdma_write(res.mrc_qp, (uint64_t)signal_buf, sizeof(uint64_t),
                      res.mr->lkey, peer.vaddr + 2 * max_msg_size, peer.rkey,
                      0 /* signaled */, 0x100 /* wr_id */);
    } else {
      volatile size_t *signal_buf =
          (uint64_t *)((char *)res.buffer + 2 * max_msg_size);
      uint64_t poll_ctr = 0;
      while (1) {
        if (msg_size_ctr == *signal_buf) {
          break;
        } else {
          poll_ctr++;
        }
      }
      MRC_LOG_DEBUG("SYNC poll_ctr=%ld\n", poll_ctr);
    }
    MRC_LOG_DEBUG("SYNC POINT: completed");
    if (is_client) {
      uint64_t peer_recv_addr = peer.vaddr + max_msg_size;
      if ((rc = warmup_sender(&res, cur_msg_size, peer_recv_addr, peer.rkey,
                              warmup_iters)))
        break;

      uint64_t peer_signal_addr =
          peer.vaddr + 2 * max_msg_size + sizeof(uint64_t);
      volatile uint64_t *local_signal_addr =
          (uint64_t *)((void *)res.buffer + 2 * max_msg_size +
                       sizeof(uint64_t));
      *local_signal_addr = msg_size_ctr;
      if ((rc = run_benchmark_sender(
               &res, cur_msg_size, iters, peer_recv_addr, peer_signal_addr,
               (uint64_t)local_signal_addr, peer.rkey, &st)))
        break;
    } else {
