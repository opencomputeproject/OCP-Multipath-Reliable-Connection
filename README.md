...
* **Protocol Restrictions:** MRC API only supports Write and Write IMM operations. **READ, SEND, or ATOMIC operations are not** permitted. Additionally, buffers for IMM operations are expected to be posted before the application starts. Therefore, RNR (Receiver Not Ready) is not supported and the hardware must ensure that a maximum configured IMM operations are outstanding. 
