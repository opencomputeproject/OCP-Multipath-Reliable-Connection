...
struct mrc_attr {
	...
	struct {
		uint8_t max_mpr; /**< Maximum supported MPR value as requestor or responder; units = 128 PSNs. */
		uint8_t mpr_res; /**< MPR resource allocation resolution; unit = 128 PSNs. */
	} mpr_attr;
 ...
 * ----------        -------------------
 * RTR                MRC_QP_MAX_WIMM_DEST, MRC_QP_MPR
 *
 * RTS                MRC_QP_WIMM, MRC_QP_RETRY_CNT, MRC_QP_TIMEOUT,
...
struct mrc_qp_attr {
	...
	struct {
		uint8_t mpr; /**< Requestor MPR value; unit=128 PSNs. */
		uint8_t mpr_dest; /**< Responder MPR value; unit=128 PSNs. */
		bool dynamic_mpr_dest; /**< If true, enable responder dynamic MPR support. */
	} mpr;
