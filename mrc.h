...
enum mrc_attr_opt {
	...
	/*
	 * The implementation supports generated EV array where the initial
	 * values are primed explicitly, but other values are generated using
	 * the bitmask.
	 */
	MRC_OPT_CAP_EV_PRIMED_GEN_ARRAY		= (1<<4),
	/*
	 * The implementation supports updating EV allowed bits after the
	 * QP has transitioned to RTS.
	 */
	MRC_OPT_CAP_UPDATE_EV_ALLOWED_BITS_RTS	= (1<<5),
	/*
	 * The implementation supports updating EV deny list after the QP
...
enum mrc_ev_mode {
	 ...
	 */
	MRC_EV_MODE_EXP_ARRAY		= 1,
	/*
	 * App will use EVs via generated arrays
	 * (see MRC_OPT_CAP_EV_GEN_ARRAY)
	 */
	MRC_EV_MODE_GEN_ARRAY		= 2,
	/*
	 * App will use primed EV generated arrays
	 * (see MRC_OPT_CAP_EV_PRIMED_GEN_ARRAY)
	 */
	MRC_EV_MODE_PRIMED_GEN_ARRAY	= 3,
};
 ...
 * Returns 0 on success or -1 on error.
 */
int mrc_get_ev_entry(struct mrc_ev_array *ev_array,
		     int index,
		     struct mrc_ev_entry *entry);
 ...
 * Returns 0 on success or -1 on error.
 */
int mrc_get_ev_deny(struct mrc_ev_array *ev_array,
		    int index,
 ...
 * Returns 0 on success or -1 on error.
 */
int mrc_update_ev_entry(struct mrc_ev_array *ev_array,
			int index,
			struct mrc_ev_entry *entry);
 ...
 * MRC_OPT_CAP_UPDATE_EV_ALLOWED_BITS_RTS.
 *
 * @param ev_array[in]         - EV array
 ...
 *               MRC_QP_RETRY_CNT
 *               MRC_QP_TIMEOUT
 *               MRC_QP_EV_ARRAY_ALLOWED_BITS [bitmask] ||
 *                 (MRC_QP_EV_ARRAY_VALUES, MRC_QP_MAX_EV_COUNT) [array]
 *               MRC_QP_EV_ARRAY
 *               MRC_QP_EV_DENY_LIST
 *               MRC_QP_EV_EVENT_MASK
 */
